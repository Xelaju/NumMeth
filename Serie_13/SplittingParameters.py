...
class SplittingParameters(object):
    ...
    def intsplit(self, psi1, psi2, a, b, tspan, N, y0, getall=False, args1=(), args2=()):
        r"""
        ...
        for k in xrange(N):
            for j in xrange(s):
                y = psi1(a[j]*h, 1.*y, *args1)
                y = psi2(b[j]*h, 1.*y, *args2)
            if getall:
                uu += [y]
                t+= [t[-1]+h]
...
class PrepSplittingParameters(object):
    ...
    def intprepsplit(self, Beps, Abig, forBeps, forAbig, y, z, tspan, N, u0, getall=False):#, args1=(), args2=()):
        ...
        for j in xrange(p): # preprocessor
        ...
        for k in xrange(N): # kernel
            for j in xrange(s):
                u = Beps(forBeps[j]*h, 1.*u)#, *args1)
                u = Abig(forAbig[j]*h, 1.*u)#, *args2)
            if getall:
                v = 1.* u
                for j in xrange(p-1,-1,-1): # postprocessor
